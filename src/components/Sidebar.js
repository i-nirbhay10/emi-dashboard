      ...
      <nav className="flex-1 px-4 space-y-1 overflow-y-auto pb-4">
        ...
        {(canAccess('orders') || canAccess('logistics') || canAccess('products') || canAccess('categories') || canAccess('inventory')) && (
          <>
            ...
            {canAccess('orders') && (
              <Link href="/payments" onClick={onNavigate} className={`${navItemClass('/payments')} group`}>
                <svg className={iconClass('/payments')} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg>
                Payments
              </Link>
            )}
            {canAccess('logistics') && (
        ...
        <div className="p-4 border-t border-slate-100 space-y-1 shrink-0">
          ...
          {canAccess('settings') && (
            <>
              ...
              <Link href="/settings/app-versions" onClick={onNavigate} className={`${navItemClass('/settings/app-versions')} group`}>
                <svg className={iconClass('/settings/app-versions')} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>
                App Versions
              </Link>
            </>
          )}
